...
def run_track_a(workspace: Workspace, base_models_source: str, mutated_models_source: str) -> TrackResult:
    ...
    if init.returncode != 0:
        return TrackResult(success=False, stage="init", error=init.stderr)
    ...
    if made_base.returncode != 0:
        return TrackResult(success=False, stage="make-migrations-base", error=made_base.stderr)
    migrated_base = cli.run("migrate", check=False)
    if migrated_base.returncode != 0:
        return TrackResult(success=False, stage="migrate-base", error=migrated_base.stderr)
    ...
    if made.returncode != 0:
        return TrackResult(success=False, stage="make-migrations", error=made.stderr)

    # Step 4: Apply mutation migration
    migrated = cli.run("migrate", check=False)
    if migrated.returncode != 0:
        return TrackResult(success=False, stage="migrate", error=migrated.stderr)
...
def _read_latest_plan(work_dir: Path) -> dict[str, Any] | None:
    plans_dir = work_dir / ".dbwarden" / "plans"
