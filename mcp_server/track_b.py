...
def run_track_b_nuclear(workspace: Workspace, mutated_models_source: str) -> TrackResult:
    ...
    models_module = _load_models_module(workspace.work_dir / "app" / "models.py", mutated_models_source)
...
def run_track_b_incremental(
    ...
    """
    unsupported = any(mutation.get("type") not in {"add_column"} for mutation in workspace.mutations)
        ...
        with engine.begin() as connection:
            for mutation in workspace.mutations:
                if mutation.get("type") == "add_column":
                    ...
                    column_type = _sql_type_for(mutation["column_type"])
                    nullable = "" if mutation.get("nullable", True) else "NOT NULL"
                    connection.execute(
                        text(f'ALTER TABLE "{table}" ADD COLUMN "{column}" {column_type} {nullable}'.strip())
                    )
    except Exception as error:  # noqa: BLE001
        return TrackResult(success=False, stage="reference-ddl", error=str(error))
    finally:
...
def _sql_type_for(column_type: str) -> str:
    mapping = {
        "Integer": "INTEGER",
        "BigInteger": "BIGINT",
        "String": "VARCHAR",
    ...
    return mapping.get(column_type, column_type.upper())
...
def _find_declarative_base(module: types.ModuleType):
    ...
    for name in dir(module):
        ...
        if isinstance(obj, type) and hasattr(obj, "metadata") and hasattr(obj.metadata, "create_all"):
            return obj
    return None
